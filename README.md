...
**Domain:** `Marketing / SEO` · **Suite:** `E-commerce Growth Suite` · **Accent:** `#a3e635`
...
**Seo Content Brief** is a standalone, browser-based AI workflow app for **Marketing / SEO**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `keyword`<br>Keyword | text | Yes | — | Affects input: Keyword. |
...
| `country_language`<br>country/language | select | No | English | Affects customization: country/language. |
| `target_audience`<br>target audience | select | No | general | Affects customization: target audience. |
| `brand_voice`<br>brand voice | text | No | — | Affects customization: brand voice. |
| `competitors`<br>competitors | text | No | — | Affects customization: competitors. |
| `word_count`<br>word count | text | No | — | Affects customization: word count. |
| `content_type`<br>content type | text | No | — | Affects customization: content type. |
| `serp_freshness`<br>SERP freshness | text | No | — | Affects customization: SERP freshness. |
...
- **country/language**: English, Arabic, Egyptian Arabic, French, German, Spanish
- **target audience**: general, executive, technical, legal, medical, student, customer
...
uvicorn app.main:app --host 127.0.0.1 --port 9152
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9152`. |
...
- Export download
- Job history
- SERP/keyword-volume provider optional
...
```bash
curl http://127.0.0.1:9152/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9152/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9152 uvicorn app.main:app --host 0.0.0.0 --port 9152
```
...
- `plugins/seo-content-brief.json` for plugin metadata.
